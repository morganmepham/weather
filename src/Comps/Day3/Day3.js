...
const Day3 = (props) => {
    ...
    const sendTemps = temps?.slice(48, 72)
    console.log(sendTemps)
    
    const codes = weather.hourly?.weathercode
    const sendCodes = codes?.slice(48, 72)
    
    const times = weather.hourly?.time
    const sendTimes = times?.slice(48, 72)


    return (
        <div className="day-div">
            {isLoaded ? <>
            <Overview code={weather?.daily?.weathercode[2]} times = {weather?.hourly?.time} celsius={celsius} icons={[rain, sun, snow, thunder, fog]} max_wind={weather?.daily?.windspeed_10m_max[2]}/>

            <Stats sunrise = {weather?.daily?.sunrise[2]} sunset = {weather?.daily?.sunset[2]} max_temp = {weather?.daily?.temperature_2m_max[2]} min_temp = {weather?.daily?.temperature_2m_min[2]} celsius={celsius}/>
