...
const Day2 = (props) => {
    ...
    const sendTemps = temps?.slice(24, 48)
    console.log(sendTemps)
    
    const codes = weather.hourly?.weathercode
    const sendCodes = codes?.slice(24, 48)
    
    const times = weather.hourly?.time
    const sendTimes = times?.slice(24, 48)


    return (
        <div className="day-div">
            {isLoaded ? <>
            <Overview code={weather?.daily?.weathercode[1]} times = {weather?.hourly?.time} celsius={celsius} icons={[rain, sun, snow, thunder, fog]} max_wind={weather?.daily?.windspeed_10m_max[1]}/>

            <Stats sunrise = {weather?.daily?.sunrise[1]} sunset = {weather?.daily?.sunset[1]} max_temp = {weather?.daily?.temperature_2m_max[1]} min_temp = {weather?.daily?.temperature_2m_min[1]} celsius={celsius}/>
