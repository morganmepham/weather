...
const Day7 = (props) => {
    ...
    const sendTemps = temps?.slice(144, 168)
    console.log(sendTemps)
    
    const codes = weather.hourly?.weathercode
    const sendCodes = codes?.slice(144, 168)
    
    const times = weather.hourly?.time
    const sendTimes = times?.slice(144, 168)


    return (
        <div className="day-div">
            {isLoaded ? <>
            <Overview code={weather?.daily?.weathercode[6]} times = {weather?.hourly?.time} celsius={celsius} icons={[rain, sun, snow, thunder, fog]} max_wind={weather?.daily?.windspeed_10m_max[6]}/>

            <Stats sunrise = {weather?.daily?.sunrise[6]} sunset = {weather?.daily?.sunset[6]} max_temp = {weather?.daily?.temperature_2m_max[6]} min_temp = {weather?.daily?.temperature_2m_min[6]} celsius={celsius}/>
