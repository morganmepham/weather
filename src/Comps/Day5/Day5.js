...
const Day5 = (props) => {
    ...
    const sendTemps = temps?.slice(96, 120)
    console.log(sendTemps)
    
    const codes = weather.hourly?.weathercode
    const sendCodes = codes?.slice(96, 120)
    
    const times = weather.hourly?.time
    const sendTimes = times?.slice(96, 120)


    return (
        <div className="day-div">
            {isLoaded ? <>
            <Overview code={weather?.daily?.weathercode[4]} times = {weather?.hourly?.time} celsius={celsius} icons={[rain, sun, snow, thunder, fog]} max_wind={weather?.daily?.windspeed_10m_max[4]}/>

            <Stats sunrise = {weather?.daily?.sunrise[4]} sunset = {weather?.daily?.sunset[4]} max_temp = {weather?.daily?.temperature_2m_max[4]} min_temp = {weather?.daily?.temperature_2m_min[4]} celsius={celsius}/>
